  ...
  function draw(t){
    ...
    if(bi===0){                                    // 0-7 the year
      ...
      tx(`${M.peakExposed} °C  →  ${M.peakShaded} °C   ·   python run_analysis.py`,
         W/2,FT+FH*.46+88,ca);
      // Divider line
      ctx.fillStyle=`rgba(255,182,39,${.55*ca})`;
      ctx.fillRect(W/2-220,FT+FH*.46+120,440,1);
      // Portal CTA
      ctx.fillStyle=`rgba(100,220,180,${.90*ca})`; mono(17,700);
      ctx.save(); ctx.letterSpacing="2px";
      tx("wasim437.github.io/Al_SAFA",W/2,FT+FH*.46+148,ca);
      ctx.restore();
      ctx.fillStyle=`rgba(246,243,237,.50)`; mono(13,400);
      tx("LIVE PROJECT PORTAL  ·  VISIT TO CHECK EVERY FIGURE IN THIS SUBMISSION",W/2,FT+FH*.46+170,ca);
      // Author row
      ctx.fillStyle=`rgba(246,243,237,.70)`; mono(14,600);
      tx("Mohamed Wasim  ·  AI Engineer",W/2,FT+FH*.46+198,ca);
      // Contact details
      ctx.fillStyle=`rgba(246,243,237,.40)`; mono(12,400);
      tx("0565919456  ·  wasimmisaw437@gmail.com  ·  github.com/wasim437",W/2,FT+FH*.46+218,ca);
      ctx.textAlign="left";
    }
