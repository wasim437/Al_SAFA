...
sections = [
    ...
    {"type": "para", "text": (
        "Shadow length = object height ÷ tan(solar elevation angle). Solar elevation and "
    ...
    {"type": "bullets", "items": [
        "Summer noon: sun near-zenith (elevation ~88°) → shadows extremely short "
        "(~0.2m for a 6m tree) — overhead canopy/shade structures required for midday "
        "summer comfort; low objects provide almost no shade.",
        "Winter noon: sun much lower (elevation ~41°) → shadows far longer (~6.9m for "
        "a 6m tree) — walls and vertical elements become effective shade sources.",
...
docx_path = build_docx_report(
    output_path=os.path.join(OUT_DIR, "Phase1.06_Shadow_Analysis_Report.docx"),
