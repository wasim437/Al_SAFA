...
// archive/phases/, and the portal now lives in docs/. Phase JSON is resolved
// against PHASE_ROOT; portal assets and the page itself stay relative to ROOT.
const PHASE_ROOT = (() => {
  for (const base of [ROOT, path.dirname(ROOT)]) {
    for (const cand of [path.join(base, 'archive', 'phases'), base]) {
      if (fs.existsSync(path.join(cand, '01_PHASE1_EXISTING_PARK'))) return cand;
    }
  }
  return ROOT;
})();
...
const readJson = (p) => JSON.parse(fs.readFileSync(path.join(PHASE_ROOT, p), 'utf8'));
...
// The cost model was re-priced against the redesigned room schedule, so it is
// checked for internal consistency rather than against the frozen Phase 7 JSON.
// The invariant that matters is that the portal shows ONE construction cost:
// the elemental take-off, the capex tile and the O&M model's build-cost basis
// all have to be the same number.
check('the capex tile shows the elemental take-off',
      provOf('capex') === D.cost.elemental.total,
      `tile ${provOf('capex')} vs take-off ${D.cost.elemental.total}`);
check('the O&M model is based on that same construction cost',
      D.cost.om.buildCost === D.cost.elemental.total,
      `O&M basis ${D.cost.om.buildCost} vs take-off ${D.cost.elemental.total}`);
check('the opex tile shows the O&M model total',
      provOf('opex') === D.cost.om.total,
      `tile ${provOf('opex')} vs O&M ${D.cost.om.total}`);
check('the take-off is priced off the drawn areas, not a frozen schedule',
      /src\/plan\.py/.test(D.cost.elemental.source),
      D.cost.elemental.source);

// The shade figure, the room schedule and the planting are no longer checked
// against the archived phase folders. Those are frozen and describe the
// superseded straight-spine scheme; checking against them would have this test
// insist the portal go back to a design that no longer exists. They are checked
// against the live pipeline instead — models/headline_metrics.json and
// data/raw/site_zoning_schedule.csv, both regenerated by `python run_analysis.py`.
const headline = JSON.parse(
    fs.readFileSync(path.join(ROOT, '..', 'models', 'headline_metrics.json'), 'utf8'));

check('walk shade percentage matches models/headline_metrics.json',
      Math.abs(provOf('spine_shade') - headline.spine_shade_canopy_only_pct) < 0.01,
      `portal ${provOf('spine_shade')} vs pipeline ${headline.spine_shade_canopy_only_pct}`);

const scheduleCsv = fs.readFileSync(
    path.join(ROOT, '..', 'data', 'raw', 'site_zoning_schedule.csv'), 'utf8')
    .trim().split(/\r?\n/).slice(1);
check('every room in the schedule is carried into the portal',
      D.zoning.zones.length === scheduleCsv.length,
      `portal ${D.zoning.zones.length} vs schedule ${scheduleCsv.length}`);
...
check('tree count matches the planting layout the models used',
      D.planting.totalTrees === headline.trees,
      `portal ${D.planting.totalTrees} vs pipeline ${headline.trees}`);
...
// The portal draws each room's real outline, generated by src/plan.py. No room
// in this scheme is a rectangle — they are curved trapezoids struck off the
// crescent's centre — so these checks measure polygon area, not w x h. The
// earlier version multiplied bounding boxes, which on a radial plan overstates
// the corner rooms by about a factor of two.
const geoCtx = { window: {} };
vm.createContext(geoCtx);
vm.runInContext(fs.readFileSync(path.join(HERE, 'plan_geometry.js'), 'utf8'), geoCtx);
const GEO = geoCtx.window.AS2_PLAN;

const layoutSrc = portalSrc.slice(
    portalSrc.indexOf('const PLAN = window.AS2_PLAN'),
    portalSrc.indexOf('const LAYOUT = computeLayout();'));
const layoutCtx = { D, Math, window: { AS2_PLAN: GEO } };
...
const areaOf = (b) => b.parts.reduce((s, p) => s + shoelace(p), 0);

check('every room is drawn as a real polygon, not a box',
      layout.length > 0 && layout.every(b => b.parts.every(p => p.length >= 3)),
      `${layout.length} rooms drawn`);

check('every drawn room matches its scheduled area within 1 m²',
      layout.every(b => Math.abs(areaOf(b) - b.zone.area) < 1.0),
      layout.filter(b => Math.abs(areaOf(b) - b.zone.area) >= 1.0)
            .map(b => `${b.zone.name}: drawn ${areaOf(b).toFixed(1)} vs ${b.zone.area}`).join('; '));

check('every room sits inside the 150 × 100 m envelope',
      layout.every(b => b.parts.every(p => p.every(([x, y]) =>
          x >= -0.01 && y >= -0.01 && x <= 150.01 && y <= 100.01))),
      layout.filter(b => b.parts.some(p => p.some(([x, y]) =>
          x < -0.01 || y < -0.01 || x > 150.01 || y > 100.01)))
            .map(b => b.zone.name).join('; '));

// Rooms may not overlap — an overlap would mean the plan double-counts area.
// Sampled on a 1 m lattice nudged off the half-metre grid, because adjacent
// rooms are supposed to share an edge and a sample sitting exactly on one
// reads as inside both polygons.
...
for (let gx = 0; gx < 150; gx++) {
    for (let gy = 0; gy < 100; gy++) {
        const px = gx + 0.637, py = gy + 0.741;
        let n = 0;
        for (const b of layout) if (b.parts.some(p => inPoly(px, py, p))) n++;
        if (n > worstClaims) worstClaims = n;
    }
}
check('no ground is claimed by two rooms at once',
      worstClaims <= 1, `worst cell claimed ${worstClaims} times`);

const drawnArea = layout.reduce((s, b) => s + areaOf(b), 0);
check('drawn rooms cover the non-circulation programme',
      drawnArea > 11000 && drawnArea < 15001,
      `${drawnArea.toFixed(0)} m² drawn of ${D.zoning.siteArea} m²; the balance is Al ` +
      'Sikkak, the alley network, shown as the gaps between rooms');

section('4b. Plan geometry module agrees with the schedule');

check('plan_geometry.js is present and carries the crescent',
      !!GEO && GEO.crescent && GEO.crescent.radius > 0,
      GEO ? `R = ${GEO.crescent.radius} m` : 'missing');

check('the arc bows to the sagitta config asks for',
      !!GEO && Math.abs((GEO.crescent.centre[1] - GEO.crescent.radius) - 41) < 0.5,
      GEO ? `midspan y = ${(GEO.crescent.centre[1] - GEO.crescent.radius).toFixed(2)} m` : '');

check('the falaj, the loop, the alleys and the majlis are all present',
      !!GEO && GEO.falaj.length > 20 && GEO.loop.length > 20 &&
      GEO.sikkak.length > 0 && GEO.majlis.length > 0,
      GEO ? `falaj ${GEO.falaj.length} pts · loop ${GEO.loop.length} · ` +
            `sikkak ${GEO.sikkak.length} · majlis ${GEO.majlis.length}` : '');

check('the room schedule closes on the site area',
      Math.abs(D.zoning.allocated - D.zoning.siteArea) < 1.0,
      `${D.zoning.allocated} vs ${D.zoning.siteArea} m²`);
